...
%
% Build:  python tools/build_paper_pdf.py
%          (three pdflatex passes in a scratch directory, then a check that the
%          rendered PDF contains the figures the artifacts say it should)
% Needs:  elsarticle.cls, lmodern, microtype, booktabs, amsmath, hyperref
%
% Last built 2026-08-28: 20 pages, 0 errors, 0 undefined references, 1 overfull
% box at 2.9pt. All fonts Type1; nothing rasterised. tools/lint_tex.py checks
% the source structurally, which is still worth doing -- a compiler will happily
% build a document with a wrong number in it.

% authoryear, not the class default. The prose is written for it -- sentences
% like "Goodman-Bacon (2021) shows that the TWFE estimand is..." read as
% "[7] shows that..." under the numeric default, and author-date is the norm in
% this literature and at the target journal. The \bibitem labels below are
% already written in [Author(Year)] form for exactly this.
\documentclass[preprint,12pt,authoryear]{elsarticle}

\usepackage[utf8]{inputenc}
% lmodern before fontenc, and it is not cosmetic. With T1 encoding and stock
% Computer Modern, MiKTeX renders some sizes from METAFONT bitmaps -- the first
% build of this document generated and embedded ecrm1728.pk for the title, which
% is fuzzy at any zoom and wrong to submit. Latin Modern is the scalable Type1
% equivalent, so the PDF is vector throughout, and it is also what lets
% microtype's font expansion run at all (expansion refuses on bitmap fonts, and
% that refusal is a fatal error, not a warning).
\usepackage{lmodern}
...
% This document is full of things TeX cannot break: \texttt{} spans, six-figure
% counts with thousands separators, and bracketed confidence intervals in math
% mode. Six paragraphs ran into the margin, the worst by 24pt. microtype's
% protrusion and expansion absorb most of it; emergencystretch gives the line
% breaker a last resort on the rest, which is preferable to hyphenating a
% coefficient or rewriting a sentence to fit.
...
\begin{table}[htbp]
\centering
\caption{Category weights (shares of in-scope observations), calendar 2024
against the post-boundary window.}
% The spelled-out column headings ran 24pt past the margin. The weights are
% named in the caption, so the headings do not have to repeat the word.
\small
\begin{tabular}{lrrr}
\toprule
