...
- **Construction pins (each changes n silently if left ambiguous):** all "same time" comparisons in **US Eastern Time**;
  ...
  **R2 corollary (binding):** R2's δ measures the *change* in ψ at the fee and publication boundaries within our own data, and its verdicts reference our own ψ̄_R1, not BDW's. So R2 requires **one construction held constant on both sides of every boundary**, fixed before δ is computed; a construction that switched at a boundary is the one thing that would actually break identification. Level fidelity to BDW is an R1 goal, not an R2 one.
  ...
  "closing day" = calendar ET date of market close timestamp; hourly-reset series excluded by series metadata (open duration <24h), not by hand-list.

  **RESOLVED 2026-07-28 — the 63-mismatch filter does not reproduce, and that is the finding.** All four readings available from Kalshi's public fields were measured against BDW's 63/46,282 = 0.136% (`tools/measure_mismatch_filter.py`): the last-trade proxy we had been running fires **never** (by the close a price has converged to its outcome, so it found 0 in 25,803); tape-price vs Kalshi's `last_price` at ≥1c gives 4.345%, 32× too many, and only an arbitrary threshold brings it near 0.136%; settlement-value vs `result` gives 0.003%, 45× too few. The two quantities in the price reading differ *by definition*, so a disagreement is not evidence of an error — tuning its threshold to land on 63 would fit a filter whose definition we do not know to a number we do. What is implemented instead is the one unambiguous data error: Kalshi's own settlement value contradicting its own `result` (1 contract in 32,728). We therefore **retain contracts BDW dropped**; at 0.136% of their sample that cannot move any reported quantity, and it is logged as a divergence. The price comparison is kept as a **validation** rather than a filter, and it is a strong one: **95.7% of our tape-derived closing prices match Kalshi's independently reported final price to the cent** (median, p90 and p95 differences all exactly zero) — independent evidence that the tape reconstruction is right.
...
*Placeholder inventory: [VERIFY] historical bid/ask availability for the spread filter; [VERIFY] `/trades` timestamp bracketing; ~~[VERIFY] post-April-2025 maker-fee schedule~~ **RESOLVED 2026-07-28 from Kalshi's own fee-document version history, 16 dated captures archived in-repo (§3); the CFTC self-certification route was checked and does not carry the per-fill maker fee — what remains open is per-series membership after 2025-10-01, tracked in `docs/known_gaps.md`, not the schedule itself*; step-zero access result; BDW journal status at submission; R3 inclusion decision in August (after R2 verdict lock); ~~exact settlement-price field behind the 63-mismatch filter~~ **RESOLVED 2026-07-28 — no available field reproduces it; a settlement-consistency check is implemented instead and the retained contracts are logged as a divergence (§3)**; ~~Polymarket→Kalshi category mapping table version~~ **RESOLVED 2026-07-28 — v2 of `data/category_map_polymarket_kalshi.yaml` maps free text (the archive has no category column) to Kalshi's real taxonomy; 94.7% of control-window markets classified, coverage reported alongside any stratified figure**; ~~R2 primary-estimand decision~~ **RESOLVED 2026-07-28 — series-level DiD, `docs/analysis_plan.md` Addendum 3**.*
